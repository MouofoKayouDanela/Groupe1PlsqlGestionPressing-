...
CREATE OR REPLACE PACKAGE BODY PA_CONSULTATION_AGENT AS
    PROCEDURE PO_PROFIL_AGENT(nom_u UTILISATEUR.Nom_utilisateur%TYPE, id_agence AGENCE.id%TYPE) IS
        ...
        BEGIN
            ...
            LOOP 
                ...
                IF c_statut IS NULL THEN 
                    ...
                    ligne_affiche := ligne_affiche + 1;
                END IF;
            END LOOP;
            IF ligne_affiche = 0 THEN
        ...
        PROCEDURE PO_LISTE_AGENT(id_agence AGENCE.id%TYPE) IS
            ...
            ;
            l_agent les_agents%ROWTYPE;
        BEGIN
            OPEN les_agents;
            LOOP
                FETCH les_agents INTO l_agent;
                EXIT WHEN les_agents%NOTFOUND;
                IF l_agent."statut" IS NULL THEN
                    DBMS_OUTPUT.PUT_LINE(compteur||' - '||l_agent."nom_utilis");
                    compteur := compteur + 1;
                END IF;
            END LOOP;
            IF compteur = 1 THEN
